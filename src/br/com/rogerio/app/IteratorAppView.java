...
//                + "\\ListaGeralDeDados(1)-Copia.csv");
        ...
        dal = new AlunoDal ("C:\\Users\\roger\\Documents\\paises.csv");
    }

    private void imprimirDadosDoAlunoDoVetor(Aluno[] lista) {

        try {
            model.setNumRows(0);
            dal.ordenaVetorDeAlunos(lista);
            for (int pos = 0; pos < lista.length; pos++) {
                String[] linha = new String[2];
                linha[0] = lista[pos].getMatricula();
                linha[1] = lista[pos].getNome().toUpperCase();
                model.addRow(linha);
            }
        } catch (Exception ex) {
    ...
    private void imprimirDadosDoAlunoDoArrayList(ArrayList<Aluno> lista) throws Exception {

        try {
            model.setNumRows(0);
            dal.ordenaArrayDeAlunos(lista);
            for (int pos = 0; pos < lista.size(); pos++) {
                String[] linha = new String[2];
                linha[0] = lista.get(pos).getMatricula();
                linha[1] = lista.get(pos).getNome();
                model.addRow(linha);
            }
    ...
    private void jButtonListarComVetorActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonListarComVetorActionPerformed
        ...
        try {
            imprimirDadosDoAlunoDoVetor(dal.listagemDeAlunosVetor());
        } catch (Exception erro) {
    ...
    private void jButtonListarComArrayListActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonListarComArrayListActionPerformed
        // TODO add your handling code here:
        try {
            imprimirDadosDoAlunoDoArrayList(dal.listagemDeAlunosArrayList());
        } catch (Exception erro) {
            JOptionPane.showMessageDialog(null, erro.getMessage());
        }
